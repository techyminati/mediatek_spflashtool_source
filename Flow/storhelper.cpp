...
protected:
    ...
    const U64 boot_size_;
    const U64 user_size_;
};
...
private:
    ...
    static U64 CalcBootSize(const DA_REPORT_T &rep)
    {
        return rep.m_nor_flash_size;
    }

    static U64 CalcUserSize(APCore::Connection *conn)
    {
        const DA_REPORT_T &rep = conn->da_report();
...
private:
    static U64 CalcAlign(const DA_REPORT_T &rep)
    {
        return static_cast<unsigned int>(rep.m_nand_pagesize) * static_cast<unsigned int>(rep.m_nand_pages_per_block);
    }
...
private:
    ...
    static U64 CalcUserSize(APCore::Connection *conn)
    {
        const DA_REPORT_T &rep = conn->da_report();
        const U64 rsv_size = GetReservedRomSize(conn);
        return rep.m_emmc_ua_size - rsv_size;
    }
};
...
private:
    ...
    static U64 CalcUserSize(APCore::Connection *conn)
    {
        const DA_REPORT_T &rep = conn->da_report();
        const U64 rsv_size = GetReservedRomSize(conn);
        return rep.m_sdmmc_ua_size - rsv_size;
    }
};
...
private:
    ...
    static U64 CalcUserSize(APCore::Connection *conn)
    {
        const DA_REPORT_T &rep = conn->da_report();
        const U64 rsv_size = GetReservedRomSize(conn);
        return rep.m_ufs_lu2_size - rsv_size;
    }
};
...
U64 CStorHelper::BootPartSize(void) const
{
    return my_impl_!=NULL ? my_impl_->BootPartSize() : 0;
}

U64 CStorHelper::UserPartSize(void) const
{
    return my_impl_!=NULL ? my_impl_->UserPartSize() : 0;
}
