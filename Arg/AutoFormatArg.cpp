                                            ...
                                            unsigned int &addr, unsigned int &len)
{
    ...
    if(ret != S_DONE)
    {
        LOGE("FlashTool_ReadPartitionCount failed, error code: %s(%d).", StatusToString(ret), ret);
    ...
    if(ret != S_DONE)
    {
        LOGE("FlashTool_ReadPartitionInfo failed, error code: %s(%d).", StatusToString(ret), ret);
        if(part_info != NULL)
        {
            delete[] part_info;
            part_info = NULL;
