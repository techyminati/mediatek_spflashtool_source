...
bool FindSpecialUSBDevice(const USBScanArg *p_arg, USBScanResult *p_result, const char * prefer_com_port_str)
{
    assert(NULL != p_arg && NULL != p_result && NULL != prefer_com_port_str);
    bool is_found(false);
    char port_name[128] = {0};
    std::vector<USB_DEVICE_INFO> buffer(p_arg->ports);
    USB_DEVICE_INFO *p0 = static_cast<USB_DEVICE_INFO*>(&buffer[0]);
    ...
    DeviceScan scan(p0);
    if (scan.FindSpecialDeviceUSBPort(p_arg->ports, prefer_com_port_str, port_name,
