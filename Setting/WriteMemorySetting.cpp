...
QSharedPointer<APCore::ICommand> WriteMemorySetting::CreateCommand(APKey key)
{
    ...
    cmd->set_is_by_sram(is_by_sram_);
    cmd->set_is_scatter_ver2(m_is_scatter_ver2);
...
void WriteMemorySetting::LoadXML(const XML::Node &node)
{
    ...
    if(!wm_node.IsEmpty())
    {
        ...
        if(name == "write-memory-item")
        {
            ...
            try{
                QString str_is_by_sram = wm_node.GetAttribute("is_by_sram").c_str();
                is_by_sram_ = (QString::compare(str_is_by_sram, "true", Qt::CaseInsensitive) == 0);
...
void WriteMemorySetting::SaveXML(XML::Node &node) const
{
    ...
    std::ostringstream str_is_by_sram;
    str_is_by_sram<<is_by_sram_;
    wm_node.SetAttribute("is_by_sram", str_is_by_sram.str());
}
