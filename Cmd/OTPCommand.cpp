...
void OTPCommand::exec(const QSharedPointer<APCore::Connection> &conn)
{
    ...
    case OTP_READ:
        ...
        LOG("OTP Read result: %s(%d)", StatusToString(ret), ret);
        break;

    case OTP_WRITE:
        ret = FlashTool_OTP_Write(conn->FTHandle(), &otp_t);
        LOGD("data is: %s", otp_arg_.GetBromParaArg().m_param.m_otp.m_data);
        LOG("OTP Write result: %s(%d)", StatusToString(ret), ret);
        break;

    case OTP_WRITE_AND_LOCK:
        ret = FlashTool_OTP_Write(conn->FTHandle(), &otp_t);
        LOG("OTP Write and Lock, write result: %s(%d)", StatusToString(ret), ret);
        if(ret != S_DONE)
            break;
        ret = FlashTool_OTP_Lock(conn->FTHandle());
        LOG("OTP Write and Lock, lock result: %s(%d)", StatusToString(ret), ret);
        break;
