...
#define sbc_pub_key_hash2 \
    sbc_pub_key2_u.r_sbc_pub_key_hash2

#define _sbc_key2_E  \
    sbc_pub_key2_u.w_sbc_pub_key2.key_e

#define _sbc_key2_N  \
    sbc_pub_key2_u.w_sbc_pub_key2.key_n
    
#define sbc_pub_key_hash3\
    sbc_pub_key3_u.r_sbc_pub_key_hash3

#define _sbc_key3_E  \
    sbc_pub_key3_u.w_sbc_pub_key3.key_e

#define _sbc_key3_N  \
    sbc_pub_key3_u.w_sbc_pub_key3.key_n
...
void EfuseCommand::WriteAll(Connection *conn)
{
    ...
    if (S_DONE != ret)
    {
        LOGI("Write Efuse registers failed: %s(%d)",
...
void EfuseCommand::ReadAll(Connection *conn)
{
    ...
    if (!rb_file.empty())
    {
        ...
        char sbcpk2_buf[32] = { 0 };
        char sbcpk3_buf[32] = { 0 };
        ...
        secure_arg.sbc_pub_key_hash2.buf = sbcpk2_buf;
        secure_arg.sbc_pub_key_hash2.buf_len = sizeof(sbcpk2_buf);
		
        secure_arg.sbc_pub_key_hash3.buf = sbcpk3_buf;
        secure_arg.sbc_pub_key_hash3.buf_len = sizeof(sbcpk3_buf);
        ...
        if (S_DONE != ret)
        {
            LOGI("Read Efuse registers failed: %s(%d)",
                 StatusToString(ret), ret);
            THROW_APP_EXCEPTION(ret, "");
        }
        else
        ...
        bool to_write /*= true*/)
{
    ...
    DumpEfuseOpt(common, efuse_sf_boot_dis);
    DumpEfuseOpt(common, sdmmc_boot_dis);
    DumpEfuseOpt(common, sbc_pub_hash_dis);
    DumpEfuseOpt(common, sbc_pub_hash1_dis);
    DumpEfuseOpt(common, sbc_pub_hash2_dis);
    DumpEfuseOpt(common, sbc_pub_hash3_dis);
	
	// added for mt8168 efuse new item begin
	DumpEfuseOpt(common, sbc_pubk_hash_fa_dis);
	DumpEfuseOpt(common, sbc_pubk_hash1_fa_dis);
    DumpEfuseOpt(common, sbc_pubk_hash2_fa_dis);
    DumpEfuseOpt(common, sbc_pubk_hash3_fa_dis);
    DumpEfuseOpt(common, sbc_pubk_hash_mtk_fa_dis);	
	DumpEfuseOpt(common, fa_mode_en);
	// added for mt8168 efuse new item end
        ...
        bool to_write /*= true*/)
{
	...
	DumpEfuseOpt(secure, rom_cmd_dis);
    DumpEfuseOpt(secure, dbgport_lock_dis);
    DumpEfuseOpt(secure, jtag_sec_passwd_en);
    ...
    else if (secure->sbc_pub_key_hash1.buf_len > 0)
    {
     ...
     if (to_write)
    {
        DumpEfuseOpt(secure, sbc_pubk2_blow);

        if(secure->_sbc_key2_E.buf_len > 0)
        {
            LOGI("sbc_public_key2_e = %s",
                 secure->_sbc_key2_E.buf);
        }
        if(secure->_sbc_key2_N.buf_len > 0)
        {
            LOGI("sbc_public_key1_n = %s",
                 secure->_sbc_key2_N.buf);
        }
    }
    else if (secure->sbc_pub_key_hash2.buf_len > 0)
    {
        DumpBinary(secure, sbc_pub_key_hash2);
    }

    if (to_write)
    {
        DumpEfuseOpt(secure, sbc_pubk3_blow);

        if(secure->_sbc_key3_E.buf_len > 0)
        {
            LOGI("sbc_public_key3_e = %s",
                 secure->_sbc_key3_E.buf);
        }
        if(secure->_sbc_key3_N.buf_len > 0)
        {
            LOGI("sbc_public_key3_n = %s",
                 secure->_sbc_key3_N.buf);
        }
    }
    else if (secure->sbc_pub_key_hash3.buf_len > 0)
    {
        DumpBinary(secure, sbc_pub_key_hash3);
    }
}
        ...
        const Efuse_Common_Arg *common)
{
    ...
    SaveEfuseOpt(rb_file, common, efuse_sf_boot_dis);
    SaveEfuseOpt(rb_file, common, sdmmc_boot_dis);
    SaveEfuseOpt(rb_file, common, sbc_pub_hash_dis);
    SaveEfuseOpt(rb_file, common, sbc_pub_hash1_dis);
    SaveEfuseOpt(rb_file, common, sbc_pub_hash2_dis);
    SaveEfuseOpt(rb_file, common, sbc_pub_hash3_dis);
	// added for mt8168 new items begin
    SaveEfuseOpt(rb_file, common, sbc_pubk_hash_fa_dis);
    SaveEfuseOpt(rb_file, common, sbc_pubk_hash1_fa_dis);
    SaveEfuseOpt(rb_file, common, sbc_pubk_hash2_fa_dis);
    SaveEfuseOpt(rb_file, common, sbc_pubk_hash3_fa_dis);
    SaveEfuseOpt(rb_file, common, sbc_pubk_hash_mtk_fa_dis);	
	SaveEfuseOpt(rb_file, common, fa_mode_en);
	// added for mt8168 new items end
        ...
        const Efuse_Secure_Arg *secure)
{
    ...
    SaveEfuseOpt(rb_file, secure, jtag_sec_passwd_en);
    ...
    if (secure->sbc_pub_key_hash1.buf_len > 0)
    {
     ...
     if (secure->sbc_pub_key_hash2.buf_len > 0)
    {
        SaveBinary(rb_file, secure, sbc_pub_key_hash2);
    }

     if (secure->sbc_pub_key_hash3.buf_len > 0)
    {
        SaveBinary(rb_file, secure, sbc_pub_key_hash3);
    }
        ...
        const Efuse_Lock_Arg *lock)
{
    ...
    SaveEfuseOpt(rb_file, lock, oem_lock);
    SaveEfuseOpt(rb_file, lock, sbc_pubk_hash1_lock);
    SaveEfuseOpt(rb_file, lock, sbc_pubk_hash2_lock);
    SaveEfuseOpt(rb_file, lock, sbc_pubk_hash3_lock);
    SaveEfuseOpt(rb_file, lock, cust_crypt_data_lock);
