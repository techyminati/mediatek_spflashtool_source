...
void CheckRPMBCommand::exec(const QSharedPointer<APCore::Connection> &conn)
{
    ...
    if(ret != STATUS_OK)
    {
        LOGE("flashtool_get_rpmb_status() failed! ret: %s(0x%x)", StatusToString(ret), ret);
        THROW_BROM_EXCEPTION(ret, 0);
    }
    LOGI("flashtool_get_rpmb_status() , rmpb_written: %s(%d).", StatusToString(rpmb_status), rpmb_status);
