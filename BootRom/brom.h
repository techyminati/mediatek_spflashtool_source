...
LIBEXPORT int __stdcall Brom_Connect(BROM_HANDLE_T *p_brom_handle, FLASHTOOL_API_HANDLE_T  *p_ft_handle, unsigned short com_port, BBCHIP_TYPE  bbchip_type, EXT_CLOCK  ext_clock, int *p_bootstop, unsigned int ms_boot_timeout, unsigned int max_start_cmd_retry_count);
...
typedef struct {
    ...
    unsigned int    sbc_pub_hash2_dis; //-1
    unsigned int    sbc_pub_hash3_dis; //-1
#define spare_num_of_byte spare.buf_len
    SpareData       spare;
    unsigned int    pid_vid_custom_en;
    unsigned int    ufs_boot_dis; //-1
    unsigned int    efuse_sf_boot_dis;
    unsigned int    sdmmc_boot_dis;
    unsigned int    sbc_pubk_hash_fa_dis;
    unsigned int    sbc_pubk_hash1_fa_dis;
    unsigned int    sbc_pubk_hash2_fa_dis;
...
typedef struct{
    ...
    unsigned int    sbc_pubk2_blow;  //-1
    union
    {
	PublicKeyStringST w_sbc_pub_key2;  //-1// sbc_public_key argument when using Brom_WriteEfuseSecure()
	KeyHashValue r_sbc_pub_key_hash2;  // return sbc_public_key hash value when using Brom_WriteEfuseAll()
    }sbc_pub_key2_u;

    unsigned int    sbc_pubk3_blow;  //-1
    union
    {
	PublicKeyStringST w_sbc_pub_key3;  //-1// sbc_public_key argument when using Brom_WriteEfuseSecure()
	KeyHashValue r_sbc_pub_key_hash3;  // return sbc_public_key hash value when using Brom_WriteEfuseAll()
    }sbc_pub_key3_u;
    ...
    unsigned int  reserved[55]; //64-2-1-1-1-1-1-2
} Efuse_Secure_Arg;
...
typedef struct{
    ...
    unsigned int oem_lock;
    unsigned int sbc_pubk_hash1_lock; //-1
    unsigned int sbc_pubk_hash2_lock; //-1
    unsigned int sbc_pubk_hash3_lock; //-1
