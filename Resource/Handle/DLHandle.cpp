...
DLHandle::DLHandle()
{
    ...
    LOG("DL_Create: %s(%d)", StatusToString(ret), ret);
}
...
int DLHandle::AutoLoadRomImages(const std::string &scat_file)
{
    ...
    if(ret != S_DONE)
    {
        LOG("DL_AutoLoad failed, ret: %s(%d).", StatusToString(ret), ret);
    }
    return ret;
}
...
int DLHandle::LoadScatter(const std::string &scat_file)
{
    ...
    {
        LOG("DL_LoadScatter failed, ret: %s(%d).", StatusToString(ret), ret);
    }
    return ret;
}
...
int DLHandle::LoadSCIScatter(const std::string &sci_file)
{
    ...
    if(S_DONE != ret)
    {
        LOG("DL_LoadSCIScatter failed, ret: %s(%d).", StatusToString(ret), ret);
    }
...
bool DLHandle::IsComboSCIDL()
{
    ...
    if(ret != S_DONE)
    {
        LOG("Get SCI Image info failed. error code: %s(%d)", StatusToString(ret), ret);
        return false;
    }
...
bool DLHandle::GetPlatformInfo(DL_PlatformInfo &info)
{
    ...
    if(ret != S_DONE)
    {
        LOG("Get Platform Infomation failed. error code: %s(%d)", StatusToString(ret), ret);
        return false;
    }
    else
    {
        ...
        if(info.m_bbchip_type == UNKNOWN_BBCHIP_TYPE)
        {
            ...
            if (strlen(shi.platform) < sizeof(info.m_bbchip_type))
            {
                strcpy(info.m_szBBChipType, shi.platform);
            }
            else
            {
                strncpy(info.m_szBBChipType, shi.platform, sizeof(info.m_szBBChipType) - 1);
                info.m_szBBChipType[sizeof(info.m_bbchip_type)] = '\0';
            }
        }
        //end
...
bool DLHandle::GetScatterVersion(char* version)
{
    ...
    if(ret != S_DONE)
    {
        LOG("Get Scatter version failed. error code: %s(%d)", StatusToString(ret), ret);
        return false;
    }
    else
    {
...
bool DLHandle::GetScatterInfo(SCATTER_Head_Info *p_info)
{
    ...
    if(ret != S_DONE)
    {
        LOG("Get Scatter info failed. error code: %s(%d)", StatusToString(ret), ret);
        return false;
    }
    else
    {
...
bool DLHandle::LoadRomFile(const std::string &rom_file, const int rom_index)
{
    ...
    if(ret != S_DONE)
    {
        LOG("Unload before LoadRomFile failed, error code: %s(%d)", StatusToString(ret), ret);
        return false;
    }
    ...
    if(ret != S_DONE)
    {
        LOG("LoadRomFile failed, error code: %s(%d)", StatusToString(ret), ret);
        return false;
    }
    return true;
}
...
void DLHandle::EnableROM(const std::string &rom_name, bool enable)
{
 ...
 void DLHandle::EnableDAChksum(int level)
 {
     ...
     if (S_DONE != ret)
     {
        LOGE("DL_SetChecksumEnable() failed, error code: %s(%d).", StatusToString(ret), ret);
        THROW_BROM_EXCEPTION(ret,0);
     }
 }
 ...
 void DLHandle::SetStopLoadFlag(int* stop_flag)
 {
     ...
     if (S_DONE != ret)
     {
        LOGE("DL_SetStopFlag() failed, error code: %s(%d).", StatusToString(ret), ret);
        THROW_BROM_EXCEPTION(ret,0);
     }
 }
}
