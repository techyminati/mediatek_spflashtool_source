...
RBHandle::RBHandle()
{
    ...
    LOG("RB_Create: %s(%d)", StatusToString(ret), ret);
}
...
RBHandle::~RBHandle()
{
    ...
    LOGD("RB_ClearAll: %s(%d)", StatusToString(ret), ret);
    ret = RB_Destroy(&handle_t_);
    LOGD("RB_Destroy: %s(%d)", StatusToString(ret), ret);
}
...
unsigned int RBHandle::GetCount() const
{
    ...
    if(S_DONE != ret)
    {
        LOGE("RB_GetCount failed, error code: %s(%d).", StatusToString(ret), ret);
        THROW_BROM_EXCEPTION(ret,0);
    }
