...
GeneralSetting::GeneralSetting(APCore::USBPower power, bool storage_life_cycle_check)
{
    ...
    m_pclConnSetting->set_storage_life_cycle_check(storage_life_cycle_check);
    m_pclChksumSetting = QSharedPointer<APCore::ChksumSetting>(new APCore::ChksumSetting());
    m_pclChksumSetting->set_chksum_level(0);
    m_pclLogInfoSetting = QSharedPointer<APCore::LogInfoSetting>(new APCore::LogInfoSetting());
}
...
GeneralSetting::GeneralSetting(const GeneralSetting& setting)
{
    ...
    this->m_pclLogInfoSetting = setting.pclGetLogInfoSetting();
}
    ...
    while(!child.IsEmpty())
    {
        ...
        else if(name == "rom-list")
        {
            ...
            while(!rom_node.IsEmpty())
            {
                ...
                if (item.enable && !FileUtils::IsFileExist(path.toStdString()))
                {
                    LOGE("the rom path is invalid: (%s).", path.toStdString().c_str());
                    THROW_APP_EXCEPTION(S_ROM_INFO_NOT_FOUND, "");
                }
        ...
        {
            this->m_pclLogInfoSetting = QSharedPointer<APCore::LogInfoSetting>(APCore::LogInfoSetting::FromXML(child));
        }
        else
        {
...
{
    this->SaveXML_ReadBack_General(root_node);
    ...
    if(!this->m_pclArg->rom_list.empty())
    {
            ...
            it!=this->m_pclArg->rom_list.end(); ++it)
        {
            ...
            XML::Node rom_node = rom_list_node.AppendChildNode("rom",path.toLocal8Bit().constData());
    ...
    if(m_pclLogInfoSetting != NULL)
    {
        m_pclLogInfoSetting->SaveXML(root_node);
    }
}

void GeneralSetting::SaveXML_ReadBack_General(XML::Node &root_node) const
{
...
const QSharedPointer<APCore::LogInfoSetting> GeneralSetting::getLogInfoSetting(const XML::Node &general_node)
{
    QSharedPointer<APCore::LogInfoSetting> logInfoSetting = QSharedPointer<APCore::LogInfoSetting>(new APCore::LogInfoSetting());
    XML::Node child = general_node.GetFirstChildNode();
    while(!child.IsEmpty())
    {
        const std::string name = child.GetName();
        if(name == "log-info")
        {
            logInfoSetting = QSharedPointer<APCore::LogInfoSetting>(APCore::LogInfoSetting::FromXML(child));
            break;
        }
        child = child.GetNextSibling();
    }
    return logInfoSetting;
}
